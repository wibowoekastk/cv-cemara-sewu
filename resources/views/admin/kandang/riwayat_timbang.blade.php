...
<head>
    ...
    <title>Riwayat Input Timbang - Admin Panel</title>
...
<body class="bg-gray-50 text-gray-800 antialiased font-sans">
            ...
            <header class="h-20 bg-white border-b border-gray-200 flex items-center justify-between px-8 sticky top-0 z-20 shadow-sm">
                <div>
                    <div class="flex items-center gap-2 text-gray-500 text-xs mb-1">
                        ...
                        <span>Kandang</span>
                        <i class="ph-bold ph-caret-right"></i>
                        <span>Riwayat Timbang</span>
                    </div>
                    <h2 class="text-2xl font-bold text-gray-800">Riwayat Data Timbang</h2>
                </div>
                ...
                <div class="mb-6 border-b border-gray-200">
                    <div class="flex flex-wrap gap-6">
                        ...
                        <a href="{{ route('admin.kandang.riwayat.kandang') }}" class="pb-3 border-b-2 border-transparent text-gray-500 hover:text-gray-700 font-medium text-sm flex items-center gap-2 transition">
                            <i class="ph ph-warehouse text-lg"></i> Riwayat Siklus (Batch)
                        </a>
                        <!-- Tab Timbang (Aktif) -->
                        <a href="{{ route('admin.kandang.riwayat.timbang') }}" class="pb-3 border-b-2 border-cemara-600 text-cemara-700 font-bold text-sm flex items-center gap-2">
                            <i class="ph-fill ph-scales text-lg"></i> Riwayat Timbang
                        </a>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 mb-6 flex flex-col md:flex-row gap-4 items-center justify-between">
                    <form method="GET" class="flex flex-wrap gap-3 w-full md:w-auto items-center">
                        ...
                        <div class="relative w-full md:w-64">
                            ...
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama kandang..." class="w-full pl-9 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-cemara-500 outline-none">
                        </div>
                        ...
                        <table class="w-full text-left text-sm text-gray-600">
                            <thead class="bg-gray-50 text-gray-800 font-bold border-b border-gray-200">
                                <tr>
                                    ...
                                    <th class="px-6 py-4 text-center">Umur</th>
                                    <th class="px-6 py-4 text-center">Berat (gr)</th>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                ...
                                @forelse($riwayatTimbang as $index => $timbang)
                                <tr class="hover:bg-gray-50 transition group">
                                    <td class="px-6 py-4 text-center">{{ $riwayatTimbang->firstItem() + $index }}</td>
                                    
                                    <!-- Tanggal Input -->
                                    <td class="px-6 py-4">
                                        <div class="font-bold text-gray-900">{{ $timbang->created_at->format('d M Y') }}</div>
                                        <div class="text-xs text-gray-400">{{ $timbang->created_at->format('H:i') }} WIB</div>
                                    </td>
                                    ...
                                    <!-- Umur -->
                                    <td class="px-6 py-4 text-center text-gray-700 font-bold">
                                        {{ $timbang->umur_minggu }} Mgg
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-center sticky right-0 bg-white group-hover:bg-gray-50 z-10 shadow-[-4px_0_12px_-4px_rgba(0,0,0,0.05)]">
                                        <div class="flex items-center justify-center gap-2">
                                            <button class="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg transition" title="Lihat Detail">
                                ...
                                <tr>
                                    <td colspan="11" class="px-6 py-8 text-center text-gray-400 italic">
                                        Belum ada riwayat input timbang.
                                    </td>
                                </tr>
                                @endforelse
    ...
    <script>
        function confirmDelete() {
            Swal.fire({
                title: 'Hapus Riwayat?',
                text: "Fitur hapus permanen dinonaktifkan demi keamanan data.",
                icon: 'info',
