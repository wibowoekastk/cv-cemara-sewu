...
<head>
    ...
    <title>Pengaturan Akun - Owner Panel</title>
                            ...
                            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 text-center">
                                ...
                                <div class="relative w-32 h-32 mx-auto mb-4 group" 
                                    ...
                                    <div class="w-32 h-32 rounded-full border-4 border-white shadow-lg overflow-hidden bg-gray-100 relative">
                                        ...
                                        <template x-if="!photoPreview">
                                            <img src="https://ui-avatars.com/api/?name={{ urlencode(Auth::user()->name) }}&background=f59e0b&color=fff&size=200" class="w-full h-full object-cover">
                                        </template>
                                    </div>

                                    <!-- Upload Button Overlay -->
                                    <div class="absolute bottom-0 right-0">
                                        <button type="button" x-on:click.prevent="$refs.photo.click()" class="bg-gold-500 text-white p-2 rounded-full shadow-md hover:bg-gold-600 transition" title="Ganti Foto">
                                ...
                                <div class="text-xs text-left bg-gold-50 p-3 rounded-lg border border-gold-100 text-gold-800">
                                    <p class="mb-1"><span class="font-bold">Role:</span> Super User</p>
                            ...
                            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                                <div class="p-6 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
                                    <h3 class="font-bold text-gray-800 flex items-center gap-2">
                                        <i class="ph-fill ph-identification-card text-gold-600 text-xl"></i> Informasi Pribadi
                                    </h3>
                                </div>
                                ...
                                <div class="p-6 space-y-5">
                                    <div>
                                        ...
                                        <input type="text" name="name" value="{{ Auth::user()->name }}" class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-gold-500 outline-none transition font-medium">
                                    </div>
                                    <div>
                                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">Email Address</label>
                                        <input type="email" name="email" value="{{ Auth::user()->email }}" class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-gold-500 outline-none transition font-medium">
                                    </div>
                                </div>
                        ...
                        <form action="{{ route('user.password.update') }}" method="POST" class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                            ...
                            <div class="p-6 space-y-5" x-data="{ showPass: false }">
                                ...
                                <div class="flex items-center gap-2">
                                    <input type="checkbox" id="showPass" class="w-4 h-4 text-gold-600 rounded focus:ring-gold-500 border-gray-300" @click="showPass = !showPass">
